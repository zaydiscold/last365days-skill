"""Output rendering for the bundled research engine."""
...
OUTPUT_DIR = Path.home() / ".local" / "share" / "last365days" / "out"
...
def ensure_output_dir():
    ...
    try:
        ...
        OUTPUT_DIR = Path(tempfile.gettempdir()) / "last365days" / "out"
        OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
...
def render_compact(report: schema.Report, limit: int = 15, missing_keys: str = "none") -> str:
    ...
    if report.mode == "web-only":
        ...
        lines.append("- Edit `~/.config/last365days/.env` to add keys")
...
def write_outputs(
    ...
    # last365days.context.md
    with open(OUTPUT_DIR / "last365days.context.md", 'w') as f:
...
def get_context_path() -> str:
    ...
    return str(OUTPUT_DIR / "last365days.context.md")
