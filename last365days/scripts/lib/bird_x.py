...
_SYSTEM_BIRD = shutil.which("bird")
...
def is_bird_installed() -> bool:
    """Check if any Bird backend is available.

    Returns:
        True if the system bird CLI exists, or if bird-search.mjs exists and Node.js is in PATH.
    """
    if _SYSTEM_BIRD:
        return True
...
def _run_bird_search(query: str, count: int, timeout: int) -> Dict[str, Any]:
    """Run a search using the system bird CLI when available, else the vendored module.
