...
def main():
    ...
    parser.add_argument(
        "--store",
        action="store_true",
        help="Persist findings to SQLite database (~/.local/share/last365days/research.db)",
    )
    parser.add_argument(
        "--diagnose",
        action="store_true",
    ...
    if not bool(config.get("OPENAI_API_KEY")):
        source_info["reddit_skip_reason"] = "No OPENAI_API_KEY (add to ~/.config/last365days/.env)"
    if not x_source:
